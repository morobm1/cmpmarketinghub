...
/* ------------------------------------------------------------------
   SCHOLARSHIP EXPECTED RECIPIENTS PARSER
   Parses a spreadsheet with columns:
     Scholarship Name | # of Recipients | Floorplan
   Returns structured data for scholarship variance analysis.
   ------------------------------------------------------------------ */

/**
 * Parse a Scholarship Expected Recipients workbook.
 *
 * Expects columns: Scholarship Name, # of Recipients, Floorplan
 *
 * @param {object} workbook - A SheetJS workbook object (already parsed)
 * @returns {{ success: boolean, data?: Array<{scholarshipName: string, expectedCount: number, floorplan: string}>, error?: string }}
 */
function parseScholarshipExpectedRecipients(workbook) {
  var sheetName = workbook.SheetNames[0];
  if (!sheetName) {
    return { success: false, error: 'Workbook contains no sheets.' };
  }
  ...
  if (jsonRows.length === 0) {
    return { success: false, error: 'Spreadsheet is empty or has no data rows.' };
  }
  ...
  for (var i = 0; i < rawHeaders.length; i++) {
    var h = rawHeaders[i];
    headerMap[h.trim().toUpperCase()] = h;
  }

  // Find required columns
  var scholarshipCol = headerMap['SCHOLARSHIP NAME'] || null;
  var recipientsCol = headerMap['# OF RECIPIENTS'] || null;
  var floorplanCol = headerMap['FLOORPLAN'] || null;

  var missingCols = [];
  if (!scholarshipCol) missingCols.push('Scholarship Name');
  if (!recipientsCol) missingCols.push('# of Recipients');
  if (!floorplanCol) missingCols.push('Floorplan');

  if (missingCols.length > 0) {
    return { success: false, error: 'Missing required column(s): ' + missingCols.join(', ') };
  }
  ...
  for (var r = 0; r < jsonRows.length; r++) {
    ...
    var scholarshipName = (raw[scholarshipCol] == null ? '' : String(raw[scholarshipCol])).trim().replace(/\s+/g, ' ');
    var recipientsRaw = (raw[recipientsCol] == null ? '' : String(raw[recipientsCol])).trim();
    var floorplan = (raw[floorplanCol] == null ? '' : String(raw[floorplanCol])).trim().replace(/\s+/g, ' ');

    // Skip empty rows
    if (!scholarshipName && !recipientsRaw && !floorplan) {
      continue;
    }

    var expectedCount = parseInt(recipientsRaw, 10);
